    ...
    def init_db(self):
        ...
        conn.execute('''
            CREATE VIRTUAL TABLE IF NOT EXISTS logs_fts USING fts5(
                id, message, metadata, content=logs, content_rowid=rowid
            )
        ''')
    ...
    def query_logs(self, sources: List[str] = None, levels: List[str] = None, 
        ...
        try:
            ...
            if search:
                ...
                '''
                if sources or levels or since:
                    # 추가 필터와 FTS를 결합
                    query = f"SELECT * FROM ({fts_query}) AS filtered WHERE 1=1"
